...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="h-16 border-b border-[#1E2638] bg-[#0B0E14] px-3 md:px-6 flex items-center justify-between sticky top-0 z-40">
      {/* Brand & Market Status */}
      <div className="flex items-center space-x-3 md:space-x-6">
        <div className="flex items-center space-x-2.5 md:space-x-3 cursor-pointer" onClick={() => setActiveTab("terminal")}>
          <div className="h-9 w-9 rounded-lg bg-gradient-to-br from-sky-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-sky-500/20 shrink-0">
            <Compass className="h-5 w-5 text-white" />
          </div>
          <div>
            <div className="flex items-center space-x-1.5 md:space-x-2">
              <span className="font-bold tracking-tight text-white text-sm md:text-base">CryptoPilot</span>
              <span className="px-1.5 py-0.5 text-[10px] font-semibold bg-sky-500/10 text-sky-400 rounded border border-sky-500/20">
                PRO AI
              </span>
            </div>
            <div className="flex items-center space-x-1.5 text-[10px] md:text-[11px] text-[#7B849B]">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />
              <span className="hidden sm:inline">Потоки данных в реальном времени (без синтетики)</span>
      ...
      <div className="flex items-center space-x-1.5 md:space-x-2">
        <div className="flex bg-[#141A29] p-1 rounded-lg border border-[#1E2638]">
          <button
            ...
            className={`flex items-center space-x-1.5 md:space-x-2 px-2.5 md:px-3.5 py-1.5 min-h-[40px] md:min-h-[36px] rounded-md text-xs font-medium transition-all ${
              activeTab === "terminal"
          ...
          >
            ...
            <span className="hidden md:inline">Терминал и Сигналы</span>
            <span className="md:hidden">Терминал</span>
          </button>
          <button
          ...
          >
            ...
            <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 hidden md:inline">
              Model D
            </span>
          </button>
        </div>
        ...
        <button
          ...
          className="hidden sm:flex items-center space-x-2 bg-[#141A29] hover:bg-[#1E2638] text-[#7B849B] px-3 py-1.5 min-h-[40px] md:min-h-[36px] rounded-lg border border-[#1E2638] text-xs transition-colors"
        >
          <span>Поиск</span>
        ...
        <button
          onClick={onOpenSearch}
          className="sm:hidden flex items-center justify-center p-2 rounded-lg bg-[#141A29] text-[#7B849B] hover:text-white border border-[#1E2638] min-h-[40px] min-w-[40px]"
